(function () {
   ...
   * @private
   */
  define('tinymce.plugins.audiodef.Plugin', [
    'tinymce.core.Env',
    ...
    PluginManager.add('audiodef', function (editor) {
        ...
        editor.windowManager.open({
          ...
          onSubmit: function (e) {
            ...
            dialogBody.forEach((field) => {
              if (field.name && e.getData()?.[field.name]) {
                params.push(`${field.name}='${e.getData()?.[field.name]}'`);
              }
            });
            var shortcode = `<span class="shortcode shortcode--audiodef">[audiodef ${params.join(
              ' '
            )}]${selectedWords}[/audiodef]</span>`;
      ...
      editor.ui.registry.addButton('audiodef', {
        type: 'button',
        icon: 'translate',
